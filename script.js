...
// Text inputs
inputTop.addEventListener("input", () => { topText = inputTop.value; topTextDiv.textContent = topText; });
inputBottom.addEventListener("input", () => { bottomText = inputBottom.value; bottomTextDiv.textContent = bottomText; });
...
function loadImage(src){
  currentImage.src = src; currentImage.crossOrigin="anonymous";
  currentImage.onload = () => {
    canvas.width = currentImage.width; canvas.height = currentImage.height;
    topTextDiv.style.top = "10px"; bottomTextDiv.style.top = canvas.height - 60 + "px";
    topTextDiv.style.left = "0px"; bottomTextDiv.style.left = "0px";
    drawMeme();
  };
}
...
function drawMeme(){
  ...
  ctx.fillStyle=fontColor; ctx.strokeStyle="black"; ctx.lineWidth=2; ctx.textAlign="center";
